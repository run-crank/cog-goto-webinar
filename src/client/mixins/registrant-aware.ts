...
export class RegistrantAwareMixin {
  ...
  public async createRegistrant(registrant: Record<string, any>, webinarKey: string, organizerKey: string): Promise<any> {
    await this.clientReady;
    return await this.client.post(`/organizers/${organizerKey}/webinars/${webinarKey}/registrants`, registrant);
  }

  public async deleteRegistrant(registrantKey: string, webinarKey: string, organizerKey: string): Promise<any> {
    await this.clientReady;
    return await this.client.delete(`/organizers/${organizerKey}/webinars/${webinarKey}/registrants/${registrantKey}`);
  }

  public async getRegistrantByRegistrantKey(registrantKey: string, webinarKey: string, organizerKey: string): Promise<any> {
    await this.clientReady;
